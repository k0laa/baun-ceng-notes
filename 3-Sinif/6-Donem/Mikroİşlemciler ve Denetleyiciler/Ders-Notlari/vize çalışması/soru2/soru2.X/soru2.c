...
void soru2_step(void)
{
  ...
  rtb_TmpLatchAtJKFlipFlopInport1 = soru2_DW.bitsForTID0.TmpLatchAtJKFlipFlopInport1_PreviousInput;

  /* Memory generated from: '<S2>/J-K Flip-Flop' */
  rtb_TmpLatchAtJKFlipFlopInport2 = soru2_DW.bitsForTID0.TmpLatchAtJKFlipFlopInport2_PreviousInput;
   ...
   */
  if ((!rtb_OR) && (soru2_PrevZCX.JKFlipFlop_Trig_ZCE != ZERO_ZCSIG)) {
    /* CombinatorialLogic: '<S3>/Logic' incorporates:
     *  Memory: '<S3>/Memory'
     */
    rowIdx = (int16_T)(((((uint16_T)soru2_DW.bitsForTID0.Memory_PreviousInput << 1) +
                         rtb_TmpLatchAtJKFlipFlopInport1) << 1) + rtb_TmpLatchAtJKFlipFlopInport2);
    soru2_DW.Logic[0U] = soru2_ConstP.Logic_table[(uint16_T)rowIdx];
    soru2_DW.Logic[1U] = soru2_ConstP.Logic_table[(uint16_T)rowIdx + 8U];

    /* Update for Memory: '<S3>/Memory' */
    soru2_DW.bitsForTID0.Memory_PreviousInput = soru2_DW.Logic[0];
  }

  soru2_PrevZCX.JKFlipFlop_Trig_ZCE = rtb_OR;

  /* End of Outputs for SubSystem: '<S2>/J-K Flip-Flop' */

  /* S-Function (MCHP_Digital_Output_Write): '<S1>/Digital Output Write' */
  LATBbits.LATB0 = soru2_DW.Logic[0];

  /* Update for Memory generated from: '<S2>/J-K Flip-Flop' */
  soru2_DW.bitsForTID0.TmpLatchAtJKFlipFlopInport1_PreviousInput = rtb_OR;

  /* Update for Memory generated from: '<S2>/J-K Flip-Flop' */
  soru2_DW.bitsForTID0.TmpLatchAtJKFlipFlopInport2_PreviousInput = rtb_OR;
}

/* Model initialize function */
void soru2_initialize(void)
{
