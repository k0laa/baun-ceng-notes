...
#pragma config FPR = XT, FOS = PRI, FCKSMEN = CSW_FSCM_OFF
#pragma config FWPSB = WDTPSB_16, FWPSA = WDTPSA_512, WDT = WDT_OFF
#pragma config ICS = ICS_PGD
#pragma config FPWRT = PWRT_64, BODENV = BORV27, BOREN = PBOR_OFF, LPOL = PWMxL_ACT_HI, HPOL = PWMxH_ACT_HI, PWMPIN = RST_IOPIN, MCLRE = MCLR_DIS

#define MCHP_isMainFile
#include "Deney5.h"
#include "Deney5_private.h"
...
int main()
{
  ...
  TRISB = 0xFF7F;                      /* Port input (1) / output (0) */
  ...
  /* Enable Time-step */
  TMR1 = 0x03E6;                       /* Initialize Timer Value */
