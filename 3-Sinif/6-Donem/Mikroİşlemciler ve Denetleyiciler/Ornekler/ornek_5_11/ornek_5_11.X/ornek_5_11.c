/*
 * -------------------------------------------------------------------
 * MPLAB Device Blocks for Simulink v3.57b (06-Aug-2024)
 *
 *   Product Page:  https://www.mathworks.com/matlabcentral/fileexchange/71892
 *           Forum: https://forum.microchip.com/s/sub-forums?&subForumId=a553l000000J2rNAAS&forumId=a553l000000J2pvAAC&subForumName=MATLAB
 *           Wiki:  http://microchip.wikidot.com/simulink:start
 * -------------------------------------------------------------------
 * File: ornek_5_11.c
 *
 * Code generated for Simulink model 'ornek_5_11'.
 *
 * Model version                  : 1.11
 * Simulink Coder version         : 23.2 (R2023b) 01-Aug-2023
 * C/C++ source code generated on : Thu Jul 16 21:01:24 2026
 */

#include "ornek_5_11.h"
#include "rtwtypes.h"

/* Block signals and states (default storage) */
DW_ornek_5_11_T ornek_5_11_DW;
volatile uint_T ContinueTimeStep __attribute__ ((near)) = 0;/* Microchip Global Variable for single rate scheduler */

/* Model step function */
void ornek_5_11_step(void)
{
  /* local block i/o variables */
  boolean_T rtb_DigitalInput;
  int16_T rowIdx;
  boolean_T rtb_Memory;
  boolean_T rtb_Memory_k;

  /* Memory: '<Root>/Memory' */
  rtb_Memory = ornek_5_11_DW.bitsForTID0.Memory_PreviousInput;

  /* S-Function (MCHP_Digital_Output_Write): '<S1>/Digital Output Write' */
  LATBbits.LATB0 = rtb_Memory;

  /* S-Function (MCHP_Digital_Input): '<Root>/Digital Input' */
  /* MCHP_Digital_Input Block: <Root>/Digital Input/Output */
  rtb_DigitalInput = PORTCbits.RC0;    /* Read pin C0 */

  /* Outputs for Enabled SubSystem: '<S4>/POSITIVE Edge' incorporates:
   *  EnablePort: '<S6>/Enable'
   */
  if (ornek_5_11_ConstB.MultiportSwitch[0] > 0.0) {
    /* RelationalOperator: '<S6>/Relational Operator1' incorporates:
     *  UnitDelay: '<S4>/Unit Delay'
     */
    ornek_5_11_DW.bitsForTID0.RelationalOperator1 = ((int16_T)rtb_DigitalInput > (int16_T)
      ornek_5_11_DW.bitsForTID0.UnitDelay_DSTATE);
  }

  /* End of Outputs for SubSystem: '<S4>/POSITIVE Edge' */

  /* Outputs for Enabled SubSystem: '<S4>/NEGATIVE Edge' incorporates:
   *  EnablePort: '<S5>/Enable'
   */
  if (ornek_5_11_ConstB.MultiportSwitch[1] > 0.0) {
    /* RelationalOperator: '<S5>/Relational Operator1' incorporates:
     *  UnitDelay: '<S4>/Unit Delay'
     */
    ornek_5_11_DW.bitsForTID0.RelationalOperator1_g = ((int16_T)ornek_5_11_DW.bitsForTID0.UnitDelay_DSTATE >
      (int16_T)rtb_DigitalInput);
  }

  /* End of Outputs for SubSystem: '<S4>/NEGATIVE Edge' */

  /* Logic: '<S4>/Logical Operator1' */
  rtb_Memory_k = (ornek_5_11_DW.bitsForTID0.RelationalOperator1 ||
                  ornek_5_11_DW.bitsForTID0.RelationalOperator1_g);

  /* CombinatorialLogic: '<S3>/Logic' incorporates:
   *  Logic: '<Root>/Logical Operator'
   *  Logic: '<Root>/Logical Operator1'
   *  Memory: '<Root>/Memory1'
   *  Memory: '<S3>/Memory'
   */
  rowIdx = (int16_T)(((((uint16_T)(ornek_5_11_DW.bitsForTID0.Memory1_PreviousInput && rtb_Memory_k) << 1) +
                       (uint16_T)(rtb_Memory_k && rtb_Memory)) << 1) +
                     ornek_5_11_DW.bitsForTID0.Memory_PreviousInput_g);
  rtb_Memory = ornek_5_11_ConstP.Logic_table[(uint16_T)rowIdx];
  rtb_Memory_k = ornek_5_11_ConstP.Logic_table[(uint16_T)rowIdx + 8U];

  /* Update for Memory: '<Root>/Memory' */
  ornek_5_11_DW.bitsForTID0.Memory_PreviousInput = rtb_Memory;

  /* Update for UnitDelay: '<S4>/Unit Delay' */
  ornek_5_11_DW.bitsForTID0.UnitDelay_DSTATE = rtb_DigitalInput;

  /* Update for Memory: '<Root>/Memory1' */
  ornek_5_11_DW.bitsForTID0.Memory1_PreviousInput = rtb_Memory_k;

  /* Update for Memory: '<S3>/Memory' */
  ornek_5_11_DW.bitsForTID0.Memory_PreviousInput_g = rtb_Memory;
}

/* Model initialize function */
void ornek_5_11_initialize(void)
{
  /* Start for S-Function (MCHP_Master): '<Root>/Microchip Master' */

  /* S-Function "Microchip MASTER" initialization Block: <Root>/Microchip Master */
}

/* Model terminate function */
void ornek_5_11_terminate(void)
{
  /* (no terminate code required) */
}

/* Scheduler */
void __attribute__((__interrupt__,__auto_psv__)) _T1Interrupt(void)
{
  {
    extern volatile uint_T ContinueTimeStep __attribute__ ((near)) ;
    __asm__ volatile ( "BSET.b %[MyVar], #0" : [MyVar] "+m" (ContinueTimeStep) ) ;/* Atomic bit-set: ContinueTimeStep |= 1U */
    _T1IF = 0;                         /* Re-enable interrupt */
  }
}

/* Single rate scheduler function */
void MCHP_Scheduler(void)
{
  for (;;) {                           /* Infinite loop, no return */
    while (!ContinueTimeStep) ;

    /* Disable Scheduler Interrupt */
    _IPL0 = 1;                         /* Disable Scheduler Interrupts. Rise IPL from 0 to 1 */
    __asm__ volatile ("BCLR.b %[MyVar], #0" : [MyVar] "+m" (ContinueTimeStep) );/* Atomic bit-clear of ContinueTimeStep bit 1U */

    /* Call model Scheduler */
    /* Step the model for base rate */
    ornek_5_11_step();

    /* Get model outputs here */

    /* Re-Enable Scheduler Interrupt */
    _IPL0 = 0;                         /* Enable Scheduler interrupt. Lower IPL from 1 to 0 */
  }                                    /* end infinite loop for(;;) */
}                                      /* End MCHP_Scheduler function */

/*
 * File trailer for generated code.
 *
 * [EOF]
 */
